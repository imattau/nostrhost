...
| §28 | Nsites (NIP-5A hosting) | ⏳ | post-alpha gateway/component spike, owner-signed publishing, safe public serving, MCP/Admin agent integration, domains and catalogue — plan in `docs/NSITES-PLAN.md`; code-level design and decisions D1–D7 in `docs/NSITES-IMPLEMENTATION-PLAN.md`. npk distribution (Phase 3c, additive): gateway `internal/npk` bundle serving (kind-9900 resolve + tar.zst unpack + content-addressed cache, per-path fallback) + `npk_enabled` gateway config + draft-area npk publish (`nsite.publish.plan --npk` / `nsite.publish --npk-release`) landed, and the bundled npack upgraded to 0.3.0 with a **local catalogue cache** (`npk.catalogue_path` ← `npack refresh`'s `catalogue.json`; release resolution reads it first, relay fallback only on a miss, SIGHUP-reloaded). Phase 4 (lifecycle + custom domains) landed: `nsite.domain.attach/detach` (TXT/CNAME ownership proof, uniqueness, Caddy on-demand TLS ask + route, gateway `custom_domains` config), immutable kind-5128 snapshots with Admin history + Snapshot action, and configure-time limit caps (`max_blob_bytes` ≤ 128 MiB, `cache_quota_bytes` ≤ 50% free space). Phase 5 (catalogue + ecosystem) largely landed: `app`-tag linkage to kind-32267 (plan → publish → site record → catalogue "open nsite" annotation, Admin wizard field), `--copy-of` `a`/`A` copies with the "Create my copy" surface, open gateway mode via the operator's ACME DNS-01 token (Admin mode selector), the **optional local Blossom component (D4)** — a loopback-only BUD-01/BUD-02 store with its own quota/per-blob/retention contract (`nsite.blossom.*`, Admin Blossom section, gateway SSRF allowance for that exact listener) — and the non-admin portal "My site" surface (D8). Remaining: Git/NIP-34 sources (D6) |
