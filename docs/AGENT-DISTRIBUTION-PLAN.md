...
- The component now contains a hardened systemd unit and Debian maintainer scripts. The APT manifest has an optional `nostrhost-agent` entry; package install does not configure, enable, or start it.
- The control relay now reconciles configured agent writer keys without granting NIP-86 admin or operation capabilities. NostrHost provides explicit `agent init/status/enable/disable`; `postinstall --new` and `--restore` leave the optional agent unconfigured.
- The first Observe config uses `service.status`, which is registered by both the agent and the current NostrHost operation dispatcher. `agent enable` validates the strict config, adds its writer key to relay config, restarts the relay, then explicitly enables the service. Operators separately grant `services.read`.
- Local Go, Python CLI, package-build, dependency-graph, and systemd-unit checks pass. VM install/upgrade/remove/purge and actual credential handoff remain unverified because SSH access to the running VM is unavailable and its QEMU guest agent is not responding.
...
1. **Implemented locally:** Extend `packaging/packages.yml` with an optional `nostrhost-agent` Go package sourced from the pinned `libs/nostrhost-agent` submodule. Do not add it to either meta-package. Depend only on the platform interfaces the agent actually uses; avoid introducing a cycle through `nostrhost-control`.
2. Extend `packaging/scripts/build-package` to fail the build if a declared unit is missing, stage the unit, and install reviewed `postinst`, `prerm`, and `postrm` lifecycle scripts (including systemd helper use and purge-only data removal). The binary package contains the daemon, its service unit, required defaults/documentation, and maintainer scripts, plus (justified by the admin UI's model management, export, and contribution flows) the `nostrhost-agent-model`, `nostrhost-agent-export`, and `nostrhost-agent-contribute` helper binaries and the `nostrhost-agent-llm.service` unit — none of these are started, enabled, or given any config by the package itself, and none are invoked by the resident daemon. Training scripts, datasets, and adapters stay excluded unless separately justified and reviewed; model weights are never bundled — only fetched, hash-verified, on explicit operator action.
3. **Implemented locally:** Add CLI/bootstrap integration in the NostrHost core: `nostrhost agent init`, `status`, `enable`, and `disable`. `postinstall --new` and `--restore` report that the optional agent is unconfigured; they do not silently create credentials, grants, enablement, or model downloads. The operator path is apt install → explicit init → inspect identity/config → separately grant scopes → explicit enable.
...
|---|---|---|---|
| A | `nostrhost-agent` component | hardened unit, service-account assumptions, systemd credential loading | daemon tests + unit validation pass; live credential test pending |
| B | umbrella packaging | optional `.deb`, unit staging, idempotent maintainer scripts, package-content check | package builds and contents are inspected; install/upgrade/purge VM test pending |
| C | NostrHost core (`forks/yunohost`) | explicit `nostrhost agent init/status/enable/disable` lifecycle, relay writer registration and scoped capability setup; no implicit `postinstall` activation | CLI tests prove identity separation, Observe default, and no automatic capability grant |
...
Community data should improve the common dataset and evaluation suite for all candidate models. By default it must not become automatic telemetry or a firehose of server logs — that stays true unless an operator explicitly opts into the automatic-submission exception described below, which they alone choose and configure per host.

This should be part of the agent project, but split across a local exporter, an explicit local submission tool, and an external review pipeline. The agent owns a companion export command because it has the verified cycle record and exact operation schemas. By default the resident daemon does not upload data, ask for a Hugging Face token, or change training labels — it never runs `nostrhost-agent-export` or `nostrhost-agent-contribute` itself. Dataset curation, review, release, and training remain maintainer-controlled processes outside the running agent.

**Automatic submission is an explicit, off-by-default exception to that boundary.** An operator can set `contribution.enabled: true` (plus `dataset_repo`) in the agent's own config — exposed in the admin UI as a separate "automatic submission" toggle, distinct from the manual-submit "sharing enabled" state — and the resident daemon will then build and submit a contribution candidate for *every* completed cycle itself, immediately, with **no human review step**. This is a deliberate, informed operator choice (confirmed explicitly when the feature was built): the automated redaction in `BuildContributionCandidate` becomes the only privacy safeguard, since no person looks at a candidate before it becomes a public pull request. Operators who want the reviewed path should leave automatic submission off and use "Prepare for review" + a manual "Submit" per candidate instead. See `agent/contribution_auto.go` (`ContributionAutoSubmitter`): it de-duplicates by cycle ID in a local state file (`contribution-submitted.jsonl`) so a restart or repeated report never re-submits the same cycle, and a submission failure is logged and does not affect the resident cycle loop.
...
2. Implemented in `nostrhost-agent-export`: it accepts one explicitly selected completed cycle, applies conservative local redaction, and writes a review candidate without transmitting data. It preserves operation schemas and leaves the expected decision unlabeled. The candidate is not dataset-ready: a maintainer workflow still needs to validate privacy, evidence, schema conversion, provenance, licensing, and acceptance. `nostrhost-agent-export --list` reports which completed cycles are export-eligible without exposing any observation/proposal content, so an operator (or the admin UI) can choose one without reading the raw journal.
2a. `nostrhost-agent-contribute` is the one tool in this project that ever transmits data off the host. It opens a pull request adding exactly one candidate file already produced by `nostrhost-agent-export` to an operator-configured Hugging Face dataset repo (Hugging Face's commit API with `create_pr=1` — it never commits directly to the target branch), authenticating with a token the operator supplies (read from a root-only file, never embedded, never logged). It is off by default (an explicit `enabled` flag plus a configured token are both required), invoked only by an explicit admin action (e.g. the admin UI's "Submit to Hugging Face" button), and refuses to run against any file that isn't a schema-tagged contribution candidate. It never reads the audit journal, never runs on a schedule, and never runs as part of the resident daemon's cycle. Landing as a PR means every submission stays in the `submitted` state described above until a maintainer reviews and merges it — the tool itself has no way to make a candidate a trusted training row.
