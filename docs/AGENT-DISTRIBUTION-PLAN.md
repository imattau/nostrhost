...
2a. `nostrhost-agent-contribute` is the one tool in this project that ever transmits data off the host. It uploads exactly one candidate file already produced by `nostrhost-agent-export` to an operator-configured Hugging Face dataset repo, authenticating with a token the operator supplies (read from a root-only file, never embedded, never logged). It is off by default (an explicit `enabled` flag plus a configured token are both required), invoked only by an explicit admin action (e.g. the admin UI's "Submit to Hugging Face" button), and refuses to run against any file that isn't a schema-tagged contribution candidate. It never reads the audit journal, never runs on a schedule, and never runs as part of the resident daemon's cycle.
