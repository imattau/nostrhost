...
| Applications and catalogue | Native catalogue list/get/publish and app install/upgrade/change-url/config tools exist in the operation registry; v1 exposes the trusted catalogue projection. Package declarations use `package.toml`; the engine creates typed resource plans. | Catalogue UI must distinguish trusted entries from local declarations, show provenance, and route every mutation through policy/approval and operation tracking. | Package schema/edit/validate/plan view and read-only catalogue endpoint are implemented; catalogue UI and apply flow remain. |
| Package authoring | Python package library, Typer CLI, schema, diagnostics, and deterministic plan are present. New API endpoints expose schema, validate, and plan. | Make this the first working browser vertical slice; allow editing and inspection only. | Implemented: NIP-07 signing, schema, TOML editor, validation feedback, and read-only plan review. |
