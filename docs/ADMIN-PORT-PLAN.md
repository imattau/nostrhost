...
1. **The native asset route is implemented.** Vite and the release manifest use `/admin/`; Caddy serves the SPA from `/usr/share/nostrhost/admin` at that path.
...
- Complete: native `/admin/` SPA route, sole NostrHost admin package definition, core recommendations, SPA build/package command, Typer package schema/plan commands, checked-in JSON Schema, and AI authoring guide.
