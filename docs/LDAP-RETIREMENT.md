...
| 14 | Policy engine irreversibility flag | `libs/nostrhost-policy/src/nostrhost_policy/policy/rules.py:190-196` | Correction: rationale is accurate, no update needed | An earlier revision of this doc claimed domains are "already LDAP-free by default (§W4)" and that this comment was stale — checked directly: the live `domains.remove` MCP tool (`yunohost_mcp/yunohost/adapter.py:2279` `domain_remove`) calls straight through to `forks/yunohost/src/domain.py`'s `domain_remove()`, which does `ldap.remove("virtualdomain=" + domain + ",ou=domains")` today. Whatever "LDAP-free native domain service" §W4 refers to is a separate, apparently not-yet-wired-in code path — it is not what the actual `domains.remove` operation executes. The policy comment's rationale stands; nothing to fix here until domain removal itself is migrated off `domain.py`. |
